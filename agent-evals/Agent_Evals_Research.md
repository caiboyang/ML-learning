...
【实践建议】这是 Wald 近似，**只在远离 0% 和 100%、且 n 不太小时可用**。边界上它会直接失效：`p = 1` 时 `SE = 0`，于是「50 道题全过」被报成一个**宽度为零的区间**，看上去像确定性结论。而**回归套件恰恰长期运行在接近 100% 的位置**——正是这个公式最不该用的地方。边界上改用 Wilson 或 Clopper–Pearson：

| 全部通过 | Wald | Wilson 95% 下界 | Clopper–Pearson 95% 下界 | 三倍法则 `1 − 3/n` |
|---|---|---|---|---|
| 50/50 | 宽度 0 | 92.9% | 92.9% | 94.0% |
| 100/100 | 宽度 0 | 96.3% | 96.4% | 97.0% |
| 200/200 | 宽度 0 | 98.1% | 98.2% | 98.5% |

【综合解释】记不住公式就记**三倍法则**：`n` 次里 0 次失败，只能说失败率大约低于 `3/n`。**100 道题全过 ≈「失败率大概在 3% 以内」，不等于「没有失败」**——按 3% 算，一天一万次真实请求就是三百次失败。
...
release:
  ...
  compare_to_baseline:                  # 必须与基线同批题、同环境快照、同 trial 数重跑
    ...
    - regression_task_flips_to_confirmed_fail == 0  # 逐题，但翻转要先复核（见下）
...
- **回归类是逐题事件**——「有没有原本可靠通过的题现在不再可靠」。单位是任务而不是总分，所以百来条的 CI 套件做回归门就够用。
  【实践建议】但**一次翻转本身不是判据**：agent 是随机的，一道题在基线通过、在候选失败，完全可能只是抽样噪声（这正是 §6.1 和学习页开篇那个 5-trial 例子说的事）。把 `flips_to_fail == 0` 直接写成 hard gate 会让 CI 变得不稳定，甚至否掉一个根本没改坏的系统。正确形状是**翻转触发复核，而不是触发失败**：
  1. 对翻转的那几道题**加跑配对 trial**（基线与候选同批、同环境、同次数）；
  2. 判据用**任务级可靠性**——例如该题的 pass 率从 ≥ 0.9 掉到 ≤ 0.6，而不是「有一次没过」；
  3. 复核后仍成立的才计入 `confirmed_fail`，才卡发布。
  这样 gate 卡的是**可靠性下降**，不是一次抽样。
...
18. Wald 区间只在远离 0%/100% 时可用；回归套件所在的边界要用 Wilson / Clopper–Pearson 或三倍法则；
19. 回归门卡的是**经复核的可靠性下降**，不是单次翻转；
20. criteria drift 说的是**在开发数据上发现欠定义/主观维度**，不是允许在留出集上改标准——客观要求与安全义务必须事先定义，且 judge 冻结后才跑发布留出集。
