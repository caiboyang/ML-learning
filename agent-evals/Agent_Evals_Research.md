...
| **Transcript** | eval 语境中一次 trial 的完整执行记录 | 模型消息、tool calls/results、关键状态快照 |
...
- **注入与 grader hacking**：把 agent 输出当不可信数据包裹，结构化输出，专门加入「要求 judge 忽略 rubric」的攻击样本。
  【实践建议】但要摆正这三条的地位：**它们是 prompt 卫生与测试手段，不是安全边界**。分隔符与 JSON schema 挡得住随手写的注入，挡不住针对性优化过的攻击——已有专门面向 LLM-as-a-judge 的优化型注入攻击研究。[S] 真正的边界是**不让 judge 的判定单独决定任何不可逆的事**，争议样本留人工裁决。
...
【综合解释】judge 通过校准之后仍是一台**有已知误差的测量仪器**，它报出来的通过率不等于真实通过率。若在留出集上量得 TPR 与 FPR，两者的关系是：

```text
观测通过率 = TPR × 真实通过率 + FPR × (1 − 真实通过率)

反解：真实通过率 = (观测通过率 − FPR) / (TPR − FPR)
```

例：TPR = 0.90、TNR = 0.85（即 FPR = 0.15），judge 在生产样本上判出 70% 通过：
