...
* 🎓 [让 Agent 跨过上下文窗口 — 从零拆解 Codex Harness](https://caiboyang.github.io/ML-learning/codex-harness/learn/)
  * *十步学习路径，用“满 100 免运费”的订单测试贯穿读文件、失败、改代码、规则更新、三路压缩与恢复验证。四份 transcript 快照配合执行循环图、三路 compaction 图和可展开练习，区分恢复规则、恢复待办与重新取证。*
...
* 📚 [Codex Harness 源码参考：上下文、Compaction 与指令加载](https://caiboyang.github.io/ML-learning/codex-harness/Codex_Harness_Research.html)（[Markdown 原文](codex-harness/Codex_Harness_Research.md)）
  * *基于 OpenAI 官方仓库固定提交 `944d6fd1ba4b` 的调用链审计，拆解执行循环、压缩与 checkpoint、WorldState 三态和 merge patch、ModelMessages 与指令 provenance。追踪 fragment 的请求元数据、history/notes 与压力提醒的启用条件，以及 Responses Lite 和 WebSocket 的缓存边界；源码事实、设计解释和未验证的服务端行为分别说明。*
