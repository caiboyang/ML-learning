...
* 🔬 [把历史扔掉，只带当前状态 — SKILL.state 论文可视化精读](https://caiboyang.github.io/ML-learning/agent-context-compression/skill-state/)
  * *围绕 arXiv:2608.26263v3，把 SKILL.state 从「更短的 prompt」还原成一套新的执行语义：每轮只给模型程序规范 P、当前状态 Σ 与最新观察 O，推理只负责产出状态补丁和下一动作。页面用可交互状态机、复杂度滑杆与四组实验图表讲清 O(T²) → O(T)、16.2× token reduction、0 步状态恢复与 budget-matched 对照；再放回本仓库的 Context Compression 六层框架，逐项比较它与 ReAct、rolling summary、LangGraph-style state、Codex / kimi-code 和事件溯源方案的差别，并单列充分统计量假设、状态无限增长、延迟相关性、审计溯源与开放权重模型结构化输出等边界。*
