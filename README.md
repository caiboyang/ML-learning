...
* 🎓 [SKILL.state：从压缩历史到维护状态 — 可视化学习页](https://caiboyang.github.io/ML-learning/agent-context-compression/skill-state/learn/)
  * *接着上面的压缩专题读 [arXiv:2608.26263](https://arxiv.org/abs/2608.26263)（EMNLP，2026-08）。用仓库管理员的比喻开场——翻工作日记 / 读一份交接笔记 / 直接读当前账本——再用立体的 P / Σ / O 状态分层、可单步的发货模拟器（含立体货架）、成本滑块和可切换的实验图表，把「从压缩历史到维护状态」讲清楚。点击式六层对照，二十个回到论文具体章节与表格的深链。*
  * *口径写得很小心：Warehouse 的 Score 是**正确动作 / 可执行事件**，不是任务成功率；CTF 的 54.2% 是**高 7.8 个百分点**不是相对增幅；字符/token 单位冲突、噪声实验只覆盖 irrelevant context、「0 步恢复」的前提是收到了纠正观察——都单独标注。*

* 📚 [SKILL.state 参考手册：全部实验数据、六条反驳与 prompt cache 门槛](https://caiboyang.github.io/ML-learning/agent-context-compression/skill-state/)
  * *与上面的学习页配套：**那篇给直觉和顺序，这篇给数字、口径和反驳**。论文 Table 1–11 全部成图（15 张交互图表，悬停查数、可展开数据表、准确率图带 ±SD 误差棒），并把 SKILL.state 放进十五平台研究的六层坐标系——结论是它**只删掉了 L2 触发，L1/L5 是简化，真正的改变是把 L3/L4 从运行时决策前移到设计期**。*
  * *六条反驳，第一条针对论文核心公式：**O(1) 只在 |Σ| 有界时成立，而论文从未验证**——固定 schema 只约束字段集合，不约束字段值长度。另含一节**论文完全没测的维度**：prompt cache 有最小可缓存前缀门槛（ADK 文档：Gemini 3 为 4,096 token），SKILL.state 恒定 ~476 token 的 prompt 结构性地够不着，而 ReAct 在第 24 步越过——让它便宜的性质正是让它拿不到缓存折扣的性质；折算后倍率 19.0× → **3–8× 区间**。*
  * *文末**更正记录**十六条，逐条列出改了什么、为什么。其中多数是我自己写错或写过头的：把推断写成来源事实、把不可比的数相除、把「论文没提」说成「论文输了」。最后三条是学习页独立读同一篇论文时先发现的——Score 的口径、Algorithm 1 先提交状态后执行动作的事务缺口、§5.7 那个不成立的推论。*
