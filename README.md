...
* 🎓 [从 API 到「MCP 2.0」— MCP 2026-07-28 零基础学习路径](https://caiboyang.github.io/ML-learning/mcp-2.0/learn/)
  * *从普通 API、RPC 与 JSON-RPC 的底层差异讲起，再还原 legacy MCP 的 Host / Client / Server、生命周期与 primitives，最后解释新设计为何走向 stateless、MRTR、显式状态 handle 与缓存，并串起 extensions、auth 和迁移。这里的「MCP 2.0」只是教学简称，官方版本是 **2026-07-28**。*
  * *与下面的参考手册配套：建议先读学习页建立问题意识和协议心智模型，再用手册核对规范、线级报文与迁移细节。*

* 📚 [MCP 2026-07-28：从 legacy MCP 到新协议模型](https://caiboyang.github.io/ML-learning/mcp-2.0/MCP_2_0_Research.html)（[Markdown 原文](mcp-2.0/MCP_2_0_Research.md)）
  * *以官方 **2026-07-28** 版本为准绳（「MCP 2.0」仅为教学简称），从 API / RPC / JSON-RPC 与 legacy MCP 的协议基础，逐层深挖 stateless 请求、MRTR、显式状态 handle、缓存、extensions、auth 和兼容迁移；配套时序图、状态机、线级示例与来源边界，适合作为查阅手册。*
