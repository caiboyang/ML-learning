...
* 📄 [把历史扔掉，只留一份可改写的状态 — SKILL.state 论文精读](https://caiboyang.github.io/ML-learning/agent-context-compression/skill-state/)
  ...
  * *还有一节**论文完全没测的维度**——prompt cache。关键是它有一条**最小可缓存前缀**门槛（ADK 文档：Gemini 3 为 4,096 token），SKILL.state 恒定 ~476 token 的 prompt **结构性地够不着**，而 ReAct 在第 24 步就越过：让它便宜的那个性质，正是让它拿不到十倍缓存折扣的那个性质。折算后倍率 19.1× → **3.3×**（建模推演，口径与更正记录已单列）。最后给出可今天就抄的**三通道混合架构**与六条按投入产出比排序的动作。*
