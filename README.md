...
* 📚 [Codex Harness 的上下文工程：Prompt 加载、World State 与三套 Compaction](https://caiboyang.github.io/ML-learning/codex-harness/Codex_Harness_Research.html)（[Markdown 原文](codex-harness/Codex_Harness_Research.md)）
  * *上面那篇横向对照十五个平台，这篇是对 `openai/codex` 的单平台纵向深挖（SHA `944d6fd1`），重心在那篇不涉及的三件事。**一、system prompt 不在客户端里**——提示词是模型描述符上的 `instructions_template` 字段、随模型从注册表下发，编译进 binary 的 `prompt.md` 只是未知 slug 的 fallback；连 tool description、审批话术、上下文提醒模板都是 model-owned。`provenance`（`Custom` vs `Model{slug}`）保证换模型时重新派生而不覆盖用户自定义；personality 是靠 H1 边界做区段切除而非模板占位符；**AGENTS.md 根本不属于 system prompt**，是带 marker 的 `user` role 消息。**二、投递路径有两条**，`use_responses_lite` 把提示词和 tool schema 降级成 `developer` role 的 input item，并用 UUIDv5 对 payload 字节做内容寻址——因为请求带 `store: false`、每轮重放整条 transcript，前缀身份稳定本身就是缓存策略。**三、`WorldState` 的 diff 机制**：每个 section 自己 snapshot、自己 `render_diff`，每轮只发变化量，落盘写 RFC 7386 merge patch；历史 append-only 改不了，于是用显式覆盖声明（"replaces all previously provided…"）作废旧文本，而三值 `PreviousSectionState`（`Known`/`Absent`/`Unknown`）决定该不该加这句声明。*
  * *compaction 一节只补那篇没有的四点：remote v2 的产物是**客户端读不了的加密 blob**（触发方式是把 `CompactionTrigger{}` 当最后一个 input item 追加）、压缩后的历史形状是**模型训练过的布局**（源码注释原话）、泄压阀 `trim_function_call_history_to_fit_context_window` 为何不算 compaction，以及重试/降级/图片预算等工程细节。另有一节讲 token-budget 模式背后的新范式：`notes` 写检查点 + `history` 检索旧窗口 + `new_context` 换窗口 + `get_context_remaining` 查预算，三级升压（剩 6,144 token 提醒 → 耗尽时动用 16,384 预留强制收尾）。在该 SHA 上 `enabled: false`，是方向而非现状。*
  * *⚠️ 本篇对上面那篇有**三处回写更新**，已同步改入那篇并标注日期与 SHA：§17.8 第 ③ 层 Codex 一栏在更晚快照上可以填上（`history` 工具即模型可调的回捞）、§2.7「要不要告诉模型上下文快满了」有了一个明确反例并给出调和路径（取决于「说了之后它有没有事可做」）、以及 §6.4 一处**实质更正**——`WorldState` 承载的是环境配置而非工具结果快照，Codex 敢丢 tool result 是因为**世界本身就是那份记录的存储处**，这个区别决定了该策略对「结果不可再生」的工具并不安全。*
...
* 🎓 [一个很小的优势，重复很多次 — 从零学量化交易](https://caiboyang.github.io/ML-learning/quantitative-trading/learn/)
  * *从零基础起步，只要求均值/方差/回归的直觉和一点 Python。开场摆两个都为真却互相打脸的事实：Medallion 基金 1988–2018 年化毛收益 **66%**，而 452 个已发表的市场异象里 **65%** 用规范口径重做后连 t &gt; 1.96 都过不了。整个专题围绕这条张力展开——量化确实有效，而寻找量化策略的过程会系统性地生产假象。*
  * *八步主线围绕唯一一条核心公式 **IR = TC × IC × √BR**（Grinold 1989 + Clarke-de Silva-Thorley 2002 的约束修正）组织：量化的取舍是牺牲单次准度（真实 IC 只有 0.02–0.06，方向正确率约 50.6%–51.9%）换取重复次数。配一个可拖动的交互控件——相关性 ρ 从 0 拨到 0.05，2000 次名义下注就塌到不足 20 次。随后是量化系统的六个模块（alpha 模型只占六分之一）、alpha 的四个来源、第一个能自己写出来的算法，以及组合构建（Markowitz 为何是「误差最大化器」、Black-Litterman、HRP、分数 Kelly）与执行成本（Almgren-Chriss 的 U 型曲线；冲击成本按 √规模 增长如何构成策略容量的硬上限）。*
